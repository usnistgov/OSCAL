...
<!-- Presumes new model (import*, merge? modify)
  ...
  <xsl:template match="profile" mode="oscal:resolve #default">
    ...
    <xsl:iterate select="*">
      <!-- $so-far starts empty -->
      <xsl:param name="so-far" as="element(resolution)"><resolution/></xsl:param>
      <xsl:on-completion select="$so-far"/>
      <xsl:next-iteration>
        <xsl:with-param name="so-far">
          <!-- now process the import, merge or modify as a proxy -->
          <xsl:apply-templates select="." mode="filter-profile">
...
<!-- import: adds control set by recursively calling profiles/catalogs
     merge: aggregates control sets given in worksheets/groups
     modify: replaces values and parameter settings -->
  
  <xsl:template match="import" as="element(resolution)" mode="filter-profile">
  ...
  <xsl:template match="merge" as="element(profile)" mode="filter-profile">
    <xsl:param name="so-far" as="element(profile)"/>
    <xsl:sequence select="$so-far"/>
  </xsl:template>
  
  <xsl:template match="import | merge | modify" as="element(profile)" mode="filter-profile">
    <xsl:param name="so-far" as="element(profile)"/>
    <xsl:sequence select="$so-far"/>
  </xsl:template>
