...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Especially for CSF of 2017 Sep -->
  <xsl:template match="oscal:component/oscal:prop[@class='number']"/>
  
  <xsl:template match="oscal:component/oscal:p">
    <p class="p">
      <xsl:for-each select="preceding-sibling::*[1]/self::oscal:prop[@class='number']">
        <span class="subst">
          <xsl:apply-templates/>
        </span>
        <xsl:text>. </xsl:text>
  ...
  <xsl:template match="oscal:control | oscal:component">
    <div class="{local-name()}">
      ...
      <xsl:call-template name="make-title">
        <xsl:with-param name="runins" select="oscal:prop[@class='function-id'] | oscal:prop[@class='category-id']"/>
      </xsl:call-template>
      <xsl:apply-templates/>
  ...
  <xsl:template match="oscal:prop[@class='function-id'] | oscal:prop[@class='category-id']"/>
...
<!-- 'insert' is a site of injection for a parameter value
  ...
  <xsl:template match="oscal:catalog | oscal:framework | oscal:section | oscal:group | oscal:control | oscal:subcontrol" mode="toc">
    <xsl:variable name="new-id">
      <xsl:apply-templates select="." mode="new-id"/>
    </xsl:variable>
    <div class="toc">
      ...
      <xsl:if test="oscal:component">
        <p class="toc-run">
          <xsl:apply-templates select="oscal:component" mode="toc"/>
        </p>
      </xsl:if>
      <xsl:apply-templates mode="toc" select="oscal:section | oscal:group | oscal:control | oscal:subcontrol"/>
    </div>
  </xsl:template>
  
  <xsl:template match="oscal:component" mode="toc">
    <xsl:variable name="new-id">
      <xsl:apply-templates select="." mode="new-id"/>
    </xsl:variable>
    <a  class="toc-link" href="#{$new-id}">
        <xsl:for-each select="oscal:prop[@class='name'] | oscal:prop[@class='number']">
          <xsl:apply-templates/>
          <xsl:text> </xsl:text>
        </xsl:for-each>
