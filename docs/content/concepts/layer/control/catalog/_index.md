...
`<param>` elements define parameter values for the control that permit (and may require) setting in context. Typically, a catalog will expose parameters where profiling applications are expected either to define appropriate values themselves (that is, values appropriate to a profile) or to permit setting at higher levels of implementation. These values are available for reference (and even display) in the language of the control, using corresponding `<insert>` elements, as described in detail below.

OSCAL parameters are worth special coverage as a topic in themselves. As shown here, they can and commonly do have internal structure and properties of their own, sometimes specific to a particular application, requirement or use case. (For example, in this data, `<prop>` elements are given to provide extra information - `alt-identifier` values -- helping assure parameters' correspondences across versions of SP 800-53.) Consumers of OSCAL content can determine where and how such information is useful to them, and ignore or discard it when it constitutes noise.
...
In this representation of AC-1, three properties are given by the data publisher. Each of these properties may be fairly judged as a "convenience value" that provides data consumers with reliable values for a control label "as published" (the string "AC-1", as distinct from the controlled machine-readable identifier for the control). It can even provide multiple values (here, two labels) in *either* or *any* required form -- or similarly, an extra identifier guaranteed to be sortable. Additionally, `<prop>` elements can be qualified by `class` to offer further distinctions from neighbor properties.

Note that by being distinguished as properties these values - both their definition and their assignment -- can be subjected to completely separate governanance and maintenance by data stakeholders from other controlled identifiers, or other values in general. Consequently these value spaces (for example: control labels of any defined type, and their formal identifers: in XPath notation, `prop[@name='label']` vs `@id`) are both semantically *distinct*, and *coordinated* -- indeed, they are coordinated exactly where and how we should expect them from their semantics to be coordinated. Because all this information is encoded together, any deviation from understood patterns (both the formalized and the informal rules) can be readily detected.
