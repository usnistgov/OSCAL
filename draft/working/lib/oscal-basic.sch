...
<!--  Contraints over declarations - very important!  -->
    <sch:rule context="oscal:control-spec//oscal:property | oscal:control-spec//oscal:statement">
      ...
      <sch:assert test="empty($named-alike except .)"><sch:name/>/@name is not unique within this control</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="oscal:prop | oscal:stmt">
      ...
      <sch:assert test="exists($catalog-entry)">
        <sch:name/> can't be named <sch:value-of select="@name"/>. <sch:value-of select="oscal:signature(.)"/>
      </sch:assert>
      ...
      <sch:assert test="empty($catalog-entry) or not($is-enumerated) or (. = $catalog-entry/oscal:value)">
        '<sch:value-of select="."/>' is not permitted as a value for property <sch:value-of select="@name"/>: try
