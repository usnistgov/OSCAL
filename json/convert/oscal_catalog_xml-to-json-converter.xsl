...
<!-- Parameter setting 'json-indent=yes' produces JSON indented using the internal serializer-->
   ...
   <xsl:strip-space elements="catalog metadata revision prop link role location address party responsible-party param constraint test guideline select control part mapping target-resource map source target group back-matter resource citation rlink"/>
