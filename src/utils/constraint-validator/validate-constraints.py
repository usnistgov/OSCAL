...
using the rules language identified by the test's 'rules-language' attribute.

Supported constraint rules languages:
...
# Well-known constraint rules language URIs
RULES_LANG_REGEX = "http://www.w3.org/TR/xmlschema-2/#regexs"
RULES_LANG_NUMERIC_RANGE = "https://csrc.nist.gov/ns/oscal/constraints/numeric-range"
...
VALIDATORS = {
    RULES_LANG_REGEX: validate_regex,
    RULES_LANG_NUMERIC_RANGE: validate_numeric_range,
}


def extract_params_with_constraints(catalog):
    """Extract parameters that have constraint tests with a rules-language attribute."""
    params = []
                        ...
                        for test in tests:
                            if "expression" in test:
                                params.append({
                                    ...
                                    "rules-language": test.get("rules-language"),
...
def validate_param(param, value):
    ...
    rules_lang = param.get("rules-language")

    if rules_lang is None:
        return {"status": "skipped", "reason": "No rules language specified (informational constraint)"}

    validator = VALIDATORS.get(rules_lang)
    if validator is None:
        return {"status": "skipped", "reason": f"Unsupported rules language: {rules_lang}"}
...
def main():
    ...
    for param in params:
        ...
        print(f"    Rules language: {param.get('rules-language', '(none)')}")
