...
export const validateContent = (
  ...
  try {
    ...
    //paths.forEach((path: string) => {
      ...
      //NEED TO FIX LINES 25-27
      const validatorFileExtension = format === 'xml' ? 'xsd' : 'json';
      const schemaFile = `validators/${format}/oscal_${model}_schema.${validatorFileExtension}`;
      const schemaValidator = `${OSCAL_DIR}build/ci-cd-test/oscal-cli-app/lib/${schemaFile}`;
      ...
      if (format === 'xml') {
        ...
        const xmlFileContent = fs.readFileSync(path);
     ...
     //   return valid
        ...
        //  console.log(validate.errors)
          console.log(`${P_ERROR}JSON Schema validation failed for '${P_END}${P_PATH}${path}${P_ERROR}' using schema '${P_END}${P_PATH}${schemaFile}${P_ERROR}'.`);
          console.log(`${path} is invalid.`);
          process.exit(1);
        } else {
          console.log(`${P_OK}JSON Schema validation passed for '${P_END}${P_PATH}${path}${P_OK}'${P_END} ${P_OK}using schema '${P_END}${P_PATH}${schemaFile}${P_OK}'.`);
        }
