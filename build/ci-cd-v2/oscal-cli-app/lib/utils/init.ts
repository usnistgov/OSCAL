...
const colorCodes = {
 ...
 const setupCliInputs = (rootDir: string, argv: any) => {
    ...
    const usage = () => `
      Usage: oscalcli ${argv._[0]} [options] [metaschema paths]
  
      -h, --help                        Display help
      -w DIR, --working_dir DIR         Generate artifacts in DIR (default: ${rootDir})
    ...
    const options = ['_', '$0', 'v', 'verbose', 'w', 'working_dir', 'workingDir', 'h', 'help'];
    ...
    if (optionDifference || argv['w'] === true || argv['working_dir'] === true) {
  ...
  const createWorkingDir = (argv: any, fileSystem: any) => {
    ...
    const parsedWorkingDir = argv['w'] || argv['workingdir'];
