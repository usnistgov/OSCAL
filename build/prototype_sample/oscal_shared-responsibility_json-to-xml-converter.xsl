...
<!-- JSON to XML conversion: pipeline -->
   ...
   <xsl:template match="j:map[@key='source-ssp']">
      ...
      <assembly name="source-ssp" key="source-ssp" gi="source-ssp">
         ...
         <xsl:apply-templates select="*[@key='last-modified']"/>
         <xsl:apply-templates select="*[@key='version']"/>
         <xsl:apply-templates select="*[@key='referenced-profile']"/>
         <xsl:apply-templates select="*[@key='props']"/>
         <xsl:apply-templates select="*[@key='links']"/>
         <xsl:apply-templates select="*[@key='remarks']"/>
      </assembly>
   </xsl:template>
   <xsl:template match="j:map[@key='referenced-profile']">
      <xsl:param name="with-key" select="true()"/>
      <!-- XML match="referenced-profile" -->
      <assembly as-type="empty"
                 name="referenced-profile"
                 key="referenced-profile"
                 gi="referenced-profile">
         <xsl:if test="$with-key">
   ...
   <xsl:template match="j:map[@key='referenced-profile']/j:string[@key='href']"
                  priority="1"><!-- XML match="referenced-profile/@href" -->
      <flag in-json="string"
             as-type="uri-reference"
             name="href"
             key="href"
